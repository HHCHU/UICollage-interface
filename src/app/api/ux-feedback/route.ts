...
export async function POST(request: NextRequest) {
  try {
    ...
    if (messages && Array.isArray(messages) && messages.length > 0) {
      ...
      const { isBaseline } = body; // 요청에서 isBaseline 값을 받아옴

      if (isBaseline) {
        // baseline 모델용 레퍼런스 분석 시나리오
        userPrompt += `\n\n제시된 9장의 레퍼런스 이미지의 장점을 분석하고, 이전에 검토한 사용자의 UI 시퀀스에 적용할 수 있는 가장 적절한 개선방안을 제시해주세요.

사용자의 첫 번째 입력 이미지와 관련된 레퍼런스(1번, 4번, 7번 이미지): 이미지들의 장점과 적용 가능한 개선점
사용자의 두 번째 입력 이미지와 관련된 레퍼런스(2번, 5번, 8번 이미지): 이미지들의 장점과 적용 가능한 개선점
사용자의 세 번째 입력 이미지와 관련된 레퍼런스(3번, 6번, 9번 이미지): 이미지들의 장점과 적용 가능한 개선점

위 레퍼런스들의 장점을 각 이미지 그룹별로 사용자의 해당 UI에 어떻게 적용할 수 있을지 구체적으로 제안해주세요.`;
      } else {
        // 기존 our 모델용 레퍼런스 분석 시나리오
